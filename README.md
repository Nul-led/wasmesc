...
Indexed writes prepend entries and update `length` when an index extends the array. Reads walk newest-to-oldest, so repeated writes are last-write-wins and holes read as `undefined`. The current array subset supports non-negative integer numeric indexes; broader JavaScript property-key coercion is not implemented yet.
