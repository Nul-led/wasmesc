...
The current `+` implementation supports number+number and string+string. Mixed string/number coercion is intentionally not implemented yet. Unary `typeof` classifies the represented runtime values as `undefined`, `boolean`, `number`, `string`, or `object` (including `null` and arrays). The JavaScript special case where `typeof` can inspect an undeclared identifier without throwing is not implemented yet; identifiers must still resolve at compile time. Unary `void` evaluates its operand once, discards the resulting JSValue, and produces `undefined`. Logical `&&`/`||` short-circuit and return operand values rather than forced booleans. Conditional `?:` also evaluates only the selected branch and returns its full JSValue.
