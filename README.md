...
The compiler also recognizes `array.push(value)`, `array.pop()`, and `array.at(index)`. `push` returns the new length; `pop` returns the removed JSValue or `undefined` for an empty array. Pop delegates to the same truncation path, so popped entries do not reappear after later length growth. `at` supports positive/negative relative numeric indexes, truncates fractions toward zero, treats NaN as zero in the current numeric subset, and returns `undefined` outside the array range.
...
- dynamic string-key coercion and additional array methods such as shift/unshift;
