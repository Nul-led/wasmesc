...
The compiler also recognizes `array.push(value)`, `array.pop()`, `array.shift()`, `array.unshift(value)`, and `array.at(index)`. `push`/`unshift` return the new length; `pop`/`shift` return the removed JSValue or `undefined` for an empty array. Shift/unshift renumber live numeric entries in place while leaving named properties untouched, so aliases continue to observe the same array object. Pop delegates to the truncation path, so removed values do not reappear after later length growth. `at` supports positive/negative relative numeric indexes, truncates fractions toward zero, treats NaN as zero in the current numeric subset, and returns `undefined` outside the array range.
