...
The current `+` implementation supports number+number and string+string. Mixed string/number coercion is intentionally not implemented yet. Logical `&&`/`||` short-circuit and return operand values rather than forced booleans.

The dynamic subset currently supports multiple top-level functions, direct calls (including forward calls and recursion), explicit exports, expression statements, numeric arithmetic and comparisons, string concatenation, short-circuit `&&`/`||`, strict equality/inequality (`===`, `!==`), unary `!`, JavaScript-style truthiness for the represented value kinds, `if`/`else`, `else if`, mutable `let`/parameter bindings, `while` loops with `break`/`continue`, primitive literals (`true`, `false`, `null`, `undefined`), interned and host-allocated strings, growable array literals with safe computed reads/writes, string-literal bracket keys, mutable `.length`, and `push`/`pop`/`shift`/`unshift`/`at`/`includes`/`indexOf`/`lastIndexOf`/`slice`/`reverse`/`fill`/`copyWithin`/`concat`, object literals, nested objects, static member reads and writes, locals, parameters, and return values. Property assignment supports new keys, overwrites, aliases, and nested member chains. Function fallthrough produces `undefined`. It is intentionally not pretending to implement all JavaScript coercion rules yet.
